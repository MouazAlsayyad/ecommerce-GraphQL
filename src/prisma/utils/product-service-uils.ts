...
import { createAttribute } from '../utils/attribute-service-utils';
import { ProductEntity, VariationOptionEntity } from 'lib/types/src';
import {
...
export async function mapProductWithVariations(
  product,
    ...
    variations.map(async (variationDto) => {
      ...
      await Promise.all(
        variationDto.variationOption.map(async (optionDto) => {
          ...
          await insertVariationOption(variation.id, optionDto.value, tx);
        }),
      );
    }),
  );
